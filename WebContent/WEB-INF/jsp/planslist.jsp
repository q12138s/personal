								...
								<tbody>
									<c:forEach items="${PlansList }" var="plan" varStatus="count">
										<tr>
											...
											<td>${plan.comment }</td>
											<td>
												<a href="${pageContext.request.contextPath }/updatePlantiaozhuan?planid=${plan.planid}
												&&planname=${plan.planname}&&plantime=${plan.plantime}&&state=${plan.state}&&remind=${plan.remind}
												">
													<i class="fa fa-edit text-navy">修改</i>
													
												</a>
												<!-- ${pageContext.request.contextPath }/delProduct?id=${pro.id} -->
												<a href="#" onclick = "delProduct('${plan.planid}')">
													<i class="fa fa-trash-o text-navy">删除</i>
												</a>
												<a href="${pageContext.request.contextPath }/Planjindutiaozhuan?planid=${plan.planid}
												&&planname=${plan.planname}&&plantime=${plan.plantime}&&state=${plan.state}&&remind=${plan.remind}
												&&jindu=${plan.jindu}
												">
												<i class="fa fa-edit text-navy">提交</i>
												</a>
											</td>
										</tr>
									</c:forEach>
